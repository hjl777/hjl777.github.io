...
- **겹침 장 넘김 (sticky-stack, 선택 사양 — 최대 1곳):** produx.design에서 관찰한 패턴의 CSS-only 이식 — 히어로를 `position: sticky; top: 0`으로 고정하고 다음 밴드가 그 위로 올라와 덮는다(음수 마진/z-index). **JS 0줄, 네이티브 스크롤 그대로** — 하이재킹 없이 "장이 물리적으로 넘어가는" 감각을 얻는 유일한 방법. 적용 후보는 홈 히어로→Projects 이음새 단 한 곳. plain 모드·프린트에서는 static으로 해제.
